...
/** ~/.ycode/v2/sessions/{workspaceHash} */
...
/** ~/.ycode/v2/sessions/{workspaceHash}/{taskId}.json */
...
/** ~/.ycode/v2/sessions/{workspaceHash}/{taskId}.deleted.json */
...
/**
 * Copy the .ycode/v2 data directory from one base dir to another.
...
export async function copyDataDirectory(oldBaseDir: string, newBaseDir: string): Promise<void> {
  const oldDir = join(oldBaseDir, ".ycode", "v2");
  const newDir = join(newBaseDir, ".ycode", "v2");
